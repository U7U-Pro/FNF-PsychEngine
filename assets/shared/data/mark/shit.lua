function onCreate()
    ...
    --chunking tron
    local mult = 0.75
    makeLuaSprite('chunk', 'chunkotron', 260, screenHeight-256*mult)
    ...
    --rating shit
    makeLuaSprite('rate', 'rating/ratingNice', getProperty('chunk.x')+512*mult, screenHeight-256*mult)
    scaleObject('rate', mult, mult)
    setObjectCamera('rate', 'camhud')
...
function onUpdateScore(miss)
    if miss == true then
        ...
        if getMisses()>9 then
            runTimer('deathTime'..getMisses(), 0.1)
        end
    end
...
function onTimerCompleted(tag, loops, loopsLeft)
    if tag == 'deathTime10' then
        setObjectOrder('packet', getObjectOrder('rate')+1)
