...
class MusicPlayer extends FlxGroup 
{
	...
	public function new(instance:FreeplayState)
	{
		...
		songTxt.setFormat(Paths.font("mingliu.ttf"), 32, FlxColor.LIME, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.RED);
		add(songTxt);

		timeTxt = new FlxText(xPos, songTxt.y + 60, 0, "", 32);
		timeTxt.setFormat(Paths.font("mingliu.ttf"), 32, FlxColor.LIME, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.RED);
		add(timeTxt);

		for (i in 0...2)
		{
			var text:FlxText = new FlxText();
			text.setFormat(Paths.font('mingliu.ttf'), 32, FlxColor.LIME, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.RED);
			text.text = '^';
			if (i == 1)
				text.flipY = true;
			text.visible = false;
		...
		playbackTxt.setFormat(Paths.font("mingliu.ttf"), 32, FlxColor.LIME , null, FlxTextBorderStyle.OUTLINE, FlxColor.RED);
		add(playbackTxt);
